...
//	std::cout << std::endl;
	...
	//pcl::PointXYZI sliced_min_pt, sliced_max_pt;
	pcl::getMinMax3D(*transform_cloud_rotate, min_pt, max_pt);
	int sectors = 10;
	double sector_height = (max_pt.z - min_pt.z)/sectors;

	pcl::PassThrough<pcl::PointXYZI> pass;
	std::vector<std::vector<double> > points_save(sectors,std::vector<double> (2));
	std::vector<std::vector<double> > slice_dist(sectors,std::vector<double> (2));


//	std::cout <<"min_pt.z : "<< min_pt.z<< ", max_pt.z : " << max_pt.z << std::endl;
//	std::cout <<"sector_height : "<< sector_height << std::endl;
	...
	for(double sec_h = min_pt.z,i = 0; sec_h < max_pt.z - sector_height; sec_h += sector_height, i++){
		...
		pass.filter(*cloud_boxel); //*cloud_sliced
		...
		//std::cout<<"cluster_indices : "<< cluster_indices.max_size() <<std::endl;
		...
		a++;
//		printf("sectors_loop : %d\n", a);

		float intensity_value[10000][2] = {0};
		...
		for(std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); (it != cluster_indices.end ()); ++it)
		{
			...
			cluster_one = it - cluster_indices.begin();
			int now_point;
			//printf("***********************************\n");
			...
			for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); pit++)
			{
				if(cloud_boxel -> points[*pit].intensity >=0){
					...
					//std::cout << conv_input -> points[*pit] << "points" <<std::endl;

					c++; // i=1
					k++;
					//printf("========================\n");

					intensity_value[k][0]= cloud_boxel -> points[*pit].x; 
					intensity_value[k][1]= cloud_boxel -> points[*pit].y;
					intensity_value[k][2]= cloud_boxel -> points[*pit].z;

					//printf("intensity_value : %d\n", k);
...
//		std::cout << c << "points" <<std::endl;

		if(k == 0){
			k = 1;
			intensity_value[k][0]= 0; 
			intensity_value[k][1]= 0;
			intensity_value[k][2]= 0;

		}

		float max_point[1] = {0}; //[0] : X, [1] : Y
		float min_point[1] = {0}; 
...
//			std::cout<<"XYZ save : ["<<j<<"]"<<"X: "<<intensity_value[j][0]<<" , Y: "<<intensity_value[j][1]<<" ,  Z: "<<intensity_value[j][2]<<std::endl;
			...
			/*if( sliced_min_pt.z > cloud_boxel -> points[*pit].z ){
				sliced_min_pt.z = cloud_boxel -> points[*pit].z;
			}
			else if( sliced_max_pt.z < cloud_boxel -> points[*pit].z ){
				sliced_max_pt.z = cloud_boxel -> points[*pit].z;
			}*/
		}
...
//		std::cout << " " << std::endl;
		...
		slice_dist[i][0] = max_point[0] - min_point[0];
		slice_dist[i][1] = max_point[1] - min_point[1];
	}
...
//	std::cout << std::endl;
	...
	if(is_save){
		std::ofstream ofs;
		ofs.open("description_1015_nega.txt", std::ios::ate | std::ios::app);
		ofs << "2 ";
...
int main (int argc, char** argv)
{
	...
	//pcd
	//ros::Subscriber sub = nh.subscribe ("output", 0, cloud_cb);
	//ros::Subscriber sub = nh.subscribe ("cloud_pcd", 0, cloud_cb);
